...
TIME_FREE_ENCODER_DICT: Dict[torch_frame.stype, Any] = {
    torch_frame.categorical: (torch_frame.nn.EmbeddingEncoder, {}),
    torch_frame.numerical: (torch_frame.nn.LinearEncoder, {}),
    torch_frame.multicategorical: (
        torch_frame.nn.MultiCategoricalEmbeddingEncoder,
...
ENCODER_DICT: Dict[str, Any] = {
    "default": DEFAULT_STYPE_ENCODER_DICT,
    "time_free": TIME_FREE_ENCODER_DICT,
}
...
class HeteroEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        for node_type in node_to_col_names_dict.keys():
            # Filter out timestamp columns if time_free is True
            if time_free:
                filtered_col_names_dict = {
                    stype: cols for stype, cols in node_to_col_names_dict[node_type].items()
                    if stype != torch_frame.timestamp
                }
                filtered_col_stats = {
                    col_name: stats for col_name, stats in node_to_col_stats[node_type].items()
                    if col_name not in node_to_col_names_dict[node_type].get(torch_frame.timestamp, [])
                }
                # Use TIME_FREE_ENCODER_DICT for time-free mode
                stype_encoder_dict = {
                    stype: TIME_FREE_ENCODER_DICT[stype][0](**TIME_FREE_ENCODER_DICT[stype][1])
                    for stype in filtered_col_names_dict.keys()
                }
            else:
                filtered_col_names_dict = node_to_col_names_dict[node_type]
                filtered_col_stats = node_to_col_stats[node_type]
                stype_encoder_dict = {
                    stype:
            ...
            self.encoders[node_type] = torch_frame_model_cls(
                **torch_frame_model_kwargs,
                ...
                col_stats=filtered_col_stats,
                col_names_dict=filtered_col_names_dict,
                stype_encoder_dict=stype_encoder_dict,
                skip_stypes=[torch_frame.timestamp] if time_free else [],
            )
...
class ZeroEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        x_dict = {
            node_type: torch.zeros(tf.num_rows, self.channels).to(tf.device)
            for node_type, tf in tf_dict.items()
        }
        return x_dict
