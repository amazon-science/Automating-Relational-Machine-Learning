...
def main(
    ...
    # Prefer RT_PRE_DIR, otherwise ~/data/pre if it exists, else fallback to ~/scratch/pre
    pre_dir = os.environ.get('RT_PRE_DIR')
    if not pre_dir:
        home = os.environ.get('HOME', '.')
        data_pre = f"{home}/data/pre"
        scratch_pre = f"{home}/scratch/pre"
        pre_dir = data_pre if os.path.exists(data_pre) else scratch_pre
    text_path = f"{pre_dir}/{dataset_name}/text.json"
    with open(text_path) as f:
        raw = f.read()
    text_list = orjson.loads(raw)

    # Handle empty text list - create empty embedding file
    if len(text_list) == 0:
        print(f"No text to embed for {dataset_name}, creating empty embedding file")
        emb_path = f"{pre_dir}/{dataset_name}/text_emb_{embedding_model}.bin"
        # Create an empty array with shape (0, 0) and save it
        emb = np.array([], dtype=bfloat16)
        emb.tofile(emb_path)
        return
