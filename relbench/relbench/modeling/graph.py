...
class NodeTrainTableInput(NamedTuple):
    ...
    - scaler_path is the path to the scaler file, for regression task 
    """
...
def transform_regression_to_binary_classification(table: pd.DataFrame, task: EntityTask) -> Tensor:
    """
    Transform a regression task to a binary classification task
    """
    nodes = torch.from_numpy(table[task.entity_col].astype(int).values)

    time: Optional[Tensor] = None
    if task.time_col is not None:
        time = torch.from_numpy(to_unix_time(table[task.time_col]))

    target: Optional[Tensor] = None
    transform: Optional[AttachTargetTransform] = None
    target = torch.from_numpy(table[[f'label_{i}' for i in range(7)]].values)
...
def get_node_train_table_input(
    ...
    target: Optional[Tensor] = None
    transform: Optional[AttachTargetTransform] = None
    scaler_path_out: Optional[str] = None
    ...
    if task.target_col in table.df:
        ...
        # Apply normalization for regression tasks if requested
        if normalize_target and is_train_scaler and (task.task_type == TaskType.REGRESSION or str(task.task_type).lower() == 'regression'):
            ...
            scaler.fit(target_np)

            # Save scaler to disk
            if scaler_path is not None:
                os.makedirs(os.path.dirname(scaler_path), exist_ok=True)
                joblib.dump(scaler, scaler_path)
                scaler_path_out = scaler_path

            # Transform target
            target_normalized = scaler.transform(target_np)
            target = torch.from_numpy(target_normalized.flatten()).float()
