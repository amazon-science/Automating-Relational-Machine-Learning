...
class UserCommentCountTask(EntityTask):
    r"""Predict the total number of comments a user will post in the next 30
    days."""
    ...
    def make_table(self, db: Database, timestamps: "pd.Series[pd.Timestamp]") -> Table:
        ...
        df = duckdb.sql(
            ...
            ON
                c.CreationDate > t.timestamp AND
                c.CreationDate <= t.timestamp + INTERVAL '{self.timedelta}'
            WHERE
                -- Only predict for users who were active in the last year
                c.UserId IN (
                    SELECT DISTINCT UserId
                    FROM comments
                    WHERE CreationDate BETWEEN t.timestamp - INTERVAL '1 year' AND t.timestamp
                )
            GROUP BY
                t.timestamp, c.UserId
            """
        ).df()
...
class PostAnswerCountTask(EntityTask):
    r"""Predict the number of new answers a question will receive in the next
    7 days."""
    ...
    def make_table(self, db: Database, timestamps: "pd.Series[pd.Timestamp]") -> Table:
        ...
        df = duckdb.sql(
            ...
            SELECT
                t.timestamp,
                questions.Id AS PostId,
                COUNT(answers.Id) AS answer_count
            FROM
                timestamp_df t
            -- Join with questions that existed at the time of the timestamp
            LEFT JOIN
                posts AS questions
            ON
                questions.PostTypeId = 1 -- Type 1 are questions
                AND questions.CreationDate <= t.timestamp
            -- Join with answers created in the future prediction window
            LEFT JOIN
                posts AS answers
            ON
                answers.ParentId = questions.Id
                AND answers.CreationDate > t.timestamp
                AND answers.CreationDate <= t.timestamp + INTERVAL '{self.timedelta}'
            WHERE
                -- Only predict for relatively recent and active questions
                questions.Id IS NOT NULL AND
                questions.CreationDate > t.timestamp - INTERVAL '6 months'
            GROUP BY
                t.timestamp, questions.Id
            """
        ).df()
