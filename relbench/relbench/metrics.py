...
def _to_binary_scores(pred: NDArray[np.float64]) -> NDArray[np.float64]:
    """Return a 1-D array of positive-class scores for binary metrics."""
    if pred.ndim == 1:
        return pred.astype(np.float64, copy=False)
    if pred.shape[1] == 1:
        return pred[:, 0].astype(np.float64, copy=False)
    if pred.shape[1] == 2:
        # assume probability/logit ordering [neg, pos]
        return pred[:, 1].astype(np.float64, copy=False)
    raise ValueError(
        "Binary metric received predictions with shape "
        f"{pred.shape}; expected 1-D scores or 2-column logits/probabilities."
    )
