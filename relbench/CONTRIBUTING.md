...
1. The dataset name is a single word (in singular), e.g. `hm`.
2. The dataset class name is the dataset name followed by `Dataset` in camel case, e.g. `HMDataset`.
3. The dataset class is defined in `relbench/datasets/<dataset_name>.py`, e.g., `relbench/datasets/hm.py`.
4. Import and register the dataset in `relbench/datasets/__init__.py` with name `rel-<dataset_name>`, e.g., `register_dataset("rel-hm", hm.HMDataset). (If you add args, you can use `rel-<dataset_name>-<qualifier>`, e.g., `register_dataset("rel-hm-fashion", hm.HMDataset, category="fashion")`)
...
{
... existing hashes ...
    "rel-hm/db.zip": "db71c7701b892a4eb7481ff04d14d25465795501dba3a5931aabee9930805efe"
}
```
...
3. The task class is defined in `relbench/tasks/<dataset_name>.py`, e.g., `relbench/tasks/hm.py`.
...
```bash
cd ~/.cache/relbench/rel-hm/tasks
...
{
... existing hashes ...
    "rel-hm/tasks/user-item-purchase.zip": "f4e2f2db27bcf30b148b4d00662101f42f68dce545f8ca57b970f534aef74c36",
    "rel-hm/tasks/user-churn.zip": "cddd511a1f609aea286cf2e20803f489ab9b3158e18cea66e0888d174b1515f0"
}
```
