...
Get a dataset, e.g., `rel-hm`:
```python
dataset: Dataset = get_dataset("rel-hm", download=True)
```
...
Various tasks can be defined on a dataset. For example, to get the `user-churn` task for `rel-hm`:
```python
task: EntityTask = get_task("rel-hm", "user-churn", download=True)
```
