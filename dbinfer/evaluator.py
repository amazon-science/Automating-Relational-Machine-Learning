...
def get_metrics(task_type: str, num_classes: int):
    if task_type == 'binary_classification':
        wanted_metrics = ["auroc", "f1", "accuracy"]
    elif task_type == 'multiclass_classification':
        wanted_metrics = ["f1", "accuracy"]
    elif task_type == 'regression':
        wanted_metrics = ["mae", "mse", "rmse", "r2"]
    elif task_type == 'retrieval':
        wanted_metrics = ["mrr", "ndcg", "map"]
    else:
        raise ValueError(f"Invalid task type {task_type}")
    metric_fns = {}
    if task_type == 'retrieval':
        for metric in wanted_metrics:
            fn = METRIC_FN[task_type][metric]
            def _retrieval_wrapper(query_idx, logits, labels, metric_fn=fn):
                with torch.no_grad():
                    preds = torch.sigmoid(logits)
                    return metric_fn(preds, labels, indexes=query_idx)
            metric_fns[metric] = _retrieval_wrapper
    elif task_type == 'binary_classification':
        for metric in wanted_metrics:
            fn = METRIC_FN[task_type][metric]
            def _classification_wrapper(seeds, logits, labels, metric_fn=fn):
                with torch.no_grad():
                    preds = F.sigmoid(logits)
                    return metric_fn(preds, labels,
                            num_classes=2,
                            task='multiclass')
            metric_fns[metric] = _classification_wrapper
    elif task_type == 'multiclass_classification':
        for metric in wanted_metrics:
            fn = METRIC_FN[task_type][metric]
            def _classification_wrapper(seeds, logits, labels, metric_fn=fn):
        ...
        for metric in wanted_metrics:
            fn = METRIC_FN[task_type][metric]
            def _regression_wrapper(seeds, logits, targets, metric_fn=fn):
                # Shape:
                #   - logits: (N,)
                #   - targets : (N,)
                with torch.no_grad():
                    return metric_fn(logits, targets.float())
            metric_fns[metric] = _regression_wrapper
    else:
        raise ValueError(f"Invalid task type {task_type}")
    return metric_fns
...
def get_metric_fn(meta : DBBTaskMeta):
    ...
    fn = METRIC_FN[meta.task_type][wanted_metric[meta.task_type]]
