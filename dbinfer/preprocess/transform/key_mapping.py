        ...
        for tbl_name, tbl in rdb_data.tables.items():
            ...
            for col_name, col in tbl.items():
                if col.metadata['dtype'] == DBBColumnDType.primary_key:
                    ...
                    for tcol_name, tcol in rdb_data.tables[target_table_name].items():
                        if tcol.metadata['dtype'] == DBBColumnDType.primary_key:
                            self.fk_to_pk[(tbl_name, col_name)] = (target_table_name, tcol_name)
                            break
                    # self.fk_to_pk[(tbl_name, col_name)] = (target_table_name, col_name)
                    self.pk_to_fk_list[(target_table_name, tcol_name)].append((tbl_name, col_name))
