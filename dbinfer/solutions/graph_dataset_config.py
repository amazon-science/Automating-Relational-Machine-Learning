...
def parse_relbench_from_graph_dataset(
    dataset : DBBGraphDataset,
    task_name : str
) -> GraphDatasetConfig:
    # Parse graph config.
    ...
    for feat_yaml in dataset.yaml_data['feature_data']:
        feat_name = feat_yaml['name']
        extra_fields = {}
        if feat_yaml['name'].endswith('numerical'):
            dtype = DBBColumnDType.float_t
            extra_fields['in_size'] = dataset.feature.size(
                feat_yaml['domain'], feat_yaml['type'], feat_yaml['name'])[0]
        elif feat_yaml['name'].endswith('categorical'):
            dtype = DBBColumnDType.category_t
            x = dataset.feature[(feat_yaml['domain'], feat_yaml['type'], feat_yaml['name'])].read()
            unique_counts = [
                torch.unique(x[:, j]).numel()
                for j in range(x.shape[1])
            ]
            extra_fields['num_categories'] = unique_counts
        elif feat_yaml['name'].endswith('timestamp'):
            dtype = DBBColumnDType.timestamp_t
        elif feat_yaml['name'].endswith('embedding'):
            dtype = DBBColumnDType.float_t
            extra_fields['num_cols'] = dataset.feature[(feat_yaml['domain'], feat_yaml['type'], feat_yaml['name'])].metadata()['num_cols']
        else:
            raise ValueError(f"Unknown feature type: {feat_yaml['name']}")
        feat_cfg = FeatureConfig(
            dtype=dtype,
            is_time=False,
            extra_fields=extra_fields
